...
class Shop
    ...
    def select_item
    	...
    	check_entry=@conn.prepare("select item_qt from item_details where item_name=?")
    	check_entry.execute(@s_prd)
    	rset=check_entry.fetch
    	ps=rset[0]

    	if(ps==0)then 
    		puts "OUT OF STOCK AVAILABLE SOON"
    		select_item
    	else
    	...
    	cart_me=@conn.prepare("INSERT INTO cart_table(cart_id,cart_name,cart_qt)SELECT item_id,item_name,item_qt FROM item_details WHERE item_name=?")
    ...
    def update_table
    	update_item=@conn.query("select temp_name from q_table")

    	while rs=update_item.fetch_row do
    ...
    def print_bill
    	create_user=@conn.query("create table if not exists user_details(user_id int AUTO_INCREMENT,user_name varchar(20),user_item varchar(20),user_qt int,user_cost int")

    	user_table=@conn.prepare("insert into user_details(user_name) ")
    	print "\n \n=========================================================================="
