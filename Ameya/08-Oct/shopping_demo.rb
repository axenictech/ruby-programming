...
class BigBazaar
	...
	def update_my_cart
		con=Mysql.connect("localhost","root","root","ShopDemo")

		puts "Please enter Item ID :"
		item_id=gets.chomp

		puts "Please enter quantity for that Item_ID :"
		qty=gets.to_i

		item_bill=0
		com=con.prepare("select Item_ID, Item_Price from InvoiceBill where Item_ID = ?")
		com.execute(item_id)
		...
		com=con.prepare("update InvoiceBill set Item_Qty=?, Item_Bill=? where Item_ID=?")
		com.execute(qty,item_bill,item_id)
	...
	def delete_from_cart
		...
		com=con.prepare("select Item_ID from InvoiceBill where Item_ID = ?")
		com.execute(item_id)

		rs = com.fetch

		com=con.prepare("delete from InvoiceBill where Item_ID=?")
		com.execute(item_id)
...
while true do
	...
	when 4
		ans.display_final_bill #show final bill first for delete
		ans.delete_from_cart #delete from my cart
		ans.display_final_bill #show final bill first for delete
