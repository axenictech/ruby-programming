...
#to create table 
	cn.query("create table if not exists\ infosys(id int, name char(20), salary int)")
	
#to insert value into table use prepare statement
	stmt=cn.prepare("insert into infosys(id,name,salary) values(?,?,?)")
...
#to fetch data from mysql value into ruby using prepare statement
	stm=cn.prepare("select * from infosys where id=?")
	stm.execute(id)
